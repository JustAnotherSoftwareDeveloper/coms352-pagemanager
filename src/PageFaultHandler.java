...
public class PageFaultHandler implements Runnable{
	...
	public void run() {
		while (!finished) {
			...
			if (nullFound) {
				System.out.println("Process "+neededAddr.getProcessNum()+" finds free frame in memory (frame number = "+nullIndex+" )");
				MemoryManger.frameTable[nullIndex]=neededAddr;
			}
			else {
				int toSwap=this.findLRUIndex();
				System.out.println("Process "+neededAddr.getProcessNum()+" replaces a frame in memory( frame number = "+toSwap+" )");
				System.out.println("Process "+neededAddr.getProcessNum()+" issues an I/O operation  to swap in demanded page (page number = "+neededAddr.getPageNum()+")");
				try {
					Thread.sleep(1000);
